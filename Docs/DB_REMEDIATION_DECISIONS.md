...
> Companion to `.bulwark/FINAL_REPORT.md`. Records which DB Audit #2 findings have been fixed in this commit and which are explicitly deferred to a pre-mainnet hardening pass, with rationale for each. Mirrors the structure of `Docs/REMEDIATION_DECISIONS.md` (the SOS audit decisions doc).
...
| **H012** | Single keypair = upgrade auth + application auth | Acknowledged pre-mainnet posture per JJ. Same finding as SOS H044/H046 (already deferred to mainnet bundle in `Docs/REMEDIATION_DECISIONS.md`). | Same plan as SOS-side: Squads multisig migration, separate keys for upgrade vs app auth. |
...
When SolShot moves to mainnet, the following work bundles need to land. Many compose with the SOS Audit #2 deferrals (see `Docs/REMEDIATION_DECISIONS.md` Section 5):
...
For SOS / on-chain decisions, see `Docs/REMEDIATION_DECISIONS.md`.
