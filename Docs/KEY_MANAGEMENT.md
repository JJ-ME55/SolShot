...
| 2026-06-04 | **🚀 MAINNET LIVE (on-chain).** v2 escrow deployed: program `BNLgn96LqskqcgTTf7cPZ5iHkaKqRdSiCdGzcAw4L7uS`, config PDA `R4u6CSnzdVbPgzcC9ukvo8bTzEH2ZF549PVGPDTGYKN` (authority/treasury/ops = the 3 Squad vaults; 7/3 fees; not paused — all verified on-chain). Deploy TX `4T2BvTYJSg2Esf9y2xjxxYh1dZXKgFQ4sdVBZCSrQXZ9dFnmr18NdfKwye5PWeYWBTcmnxQV9YcPrMrcupYeuogq`, init-config TX `2mEdRsxdQvKK4ibAMymLfvCKUD6yrvSrqTEYb8czoEeBZ1MNqjqWoSX6z3iZfQENZrjGeUwNAsLL3PS72Nji1HUa`. Upgrade authority handed to Authority Squad vault `9f1M…` (2-of-3); deployer `3sQt…` retired. Done entirely via CLI per §4 (deploy → `set-upgrade-authority --skip…`). **Pending:** Render/Vercel env cutover + mainnet smoke. | JJ + Claude |

| 2026-06-04 | **⚠️ CRITICAL DESIGN FIX — `config.authority` must be the SERVER key, NOT the Squad vault.** The program ties `create_match`/`settle_match` to `config.authority` (`has_one = authority`), so a multisig PDA can't be the config authority — the automated server signs every match. Mainnet config was mistakenly initialized with `authority = 9f1M…` (Squad vault), which made create/settle impossible. Rotated to the server key `CgcAZJf6…` via `propose_authority` (executed by the Squad through Squads TX Builder, 2-of-3) + `accept_authority` (TX `4bcHF4w6KV5GXumqXVAJ3xDdx1LK4XdQc4uVU2o6786URu1bBfBtbS5QkEb5jccutcHaYyLqAs2Ni1KN8Q21Grz5`). The Squad correctly stays the **upgrade** authority + treasury/ops fund holders. **TODO:** §1/§2/§3/§4 still say "Vault 0 = config governance" — that's WRONG; correct them (config.authority = server key; Squad = upgrade + funds only; governance via the 24h `update_config` timelock). Also fixed `accept-authority-v2.mjs` to honor `ESCROW_PROGRAM_ID_V2` (it was binding to the devnet IDL address). | JJ + Claude |
