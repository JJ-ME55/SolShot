...
1. **Goff & Carré 2010 PDF text extraction failed** in both the research session (WebFetch blocked) AND in a follow-up direct WebFetch attempt on 2026-05-15 (PDF binary-only — content was compressed, the tool returned only PDF structural data, no extractable equation text). The fit constants `c = 0.4127, d = 0.3056` from citation summaries remain unverified. **For v1 we use the Asai-2007-derived linear form `Cl(Sp) = clamp(0.2 + 0.5·(Sp − 0.18), 0.15, 0.30)` — this is research-grounded (Asai wind-tunnel slope) rather than a guess, but it's an approximation, not the canonical Goff & Carré fit.** If we ever want exactness, options: (a) get the PDF through institutional access and copy the equation, (b) use OCR on the saved binary, (c) ask main-claude / John to verify (the comms entry asks this).
