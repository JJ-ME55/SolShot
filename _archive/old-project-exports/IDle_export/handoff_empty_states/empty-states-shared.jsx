/* global React */
// SolShot — Empty / Loading / Error state primitives
// Field-manual aesthetic. Tokens-only. No new fonts. No border-radius. clip-path only.
//
// Three primitives:
//   <EmptyState icon title body primaryCTA secondaryCTA />
//   <SkeletonRow height="…" lines="…" />     <SkeletonCard width height />
//   <ErrorState icon title body primaryCTA="RETRY" secondaryCTA />
//
// Sizing:
//   The components fill their parent box (position: absolute, inset: 0)
//   so they can drop straight into any screen-content region without the
//   caller fighting layout.

const ES = {
  // Color slots (read from tokens at runtime via CSS vars; constants only here for reference)
  bone: "var(--bone)",
  olive: "var(--olive)",
  muted: "var(--muted)",
  accent: "var(--accent)",
...
// ─────────────────────────────────────────────────────────────
// ICON SET — 12 pixel-art SVGs in field-manual style
// All draw on a 24×24 grid with currentColor strokes/fills.
// Stroke-only where possible; tiny solid pixels for emphasis.
// ─────────────────────────────────────────────────────────────
const ICON_VIEWBOX = "0 0 24 24";
const stroke = (extra = {}) => ({ fill: "none", stroke: "currentColor", strokeWidth: 1.25, strokeLinecap: "square", strokeLinejoin: "miter", shapeRendering: "crispEdges", ...extra });
const px = (extra = {}) => ({ fill: "currentColor", shapeRendering: "crispEdges", ...extra });

function Icon({ name, size = 56, color = "var(--olive)", style = {} }) {
...
const ICONS = {
  ...
  // Reticle / crosshair — barracks "no kills yet"
  reticle: (
    <g>
  ...
  // Magnifying glass — search / not-found
  search: (
    <g>
  ...
  // Lock — locked tier / wallet not connected
  lock: (
    <g>
  ...
  // Target / pin — leaderboard "no operatives"
  target: (
    <g>
      <circle cx="12" cy="12" r="8" {...stroke()} />
  ...
  // Wallet / link broken — connect wallet
  wallet: (
    <g>
  ...
  // Warning triangle — error generic
  warning: (
    <g>
  ...
  // Transmission failure — radar burst with X
  txfail: (
    <g>
  ...
  // Skull crossbones-ish — challenge expired
  skull: (
    <g>
...
// ─────────────────────────────────────────────────────────────
// CTA Buttons — match tokens.css .btn family but inline (no external CSS reach)
// ─────────────────────────────────────────────────────────────
function CTA({ kind = "primary", onClick, children, compact = false, fullWidth = false }) {
  const base = {
  ...
  if (kind === "primary") {
    return <button onClick={onClick} style={{ ...base, background: "var(--accent)", color: "#0e1209" }}>{children}</button>;
  }
  if (kind === "danger") {
    return <button onClick={onClick} style={{ ...base, background: "var(--accent)", color: "#0e1209" }}>{children}</button>;
  }
...
// ─────────────────────────────────────────────────────────────
// EmptyState
//   icon: name from ICONS or React node
//   title: short, uppercase, Black Ops
//   body: 1-line, Share Tech Mono, olive
//   primaryCTA: { label, onClick } | null
//   secondaryCTA: { label, onClick } | null
//   density: "regular" | "compact"  (compact halves padding for tight regions)
//   tone: "default" | "error"  (error swaps title color to red, used by ErrorState wrapper)
// ─────────────────────────────────────────────────────────────
function EmptyState({
  icon = "radar",
  iconColor,
  ...
  density = "regular",
  tone = "default",
  bracketed = true,        // corner brackets framing the icon
}) {
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        {bracketed && [0, 1, 2, 3].map(i => <CornerBracket key={i} pos={i} color={isError ? "var(--red)" : "var(--muted)"} />)}
        <Icon
      ...
      {(primaryCTA || secondaryCTA) && (
        <div style={{
          display: "flex", gap: 8, marginTop: compact ? 4 : 10,
...
// ─────────────────────────────────────────────────────────────
// ErrorState — same chassis as EmptyState, red accents, default RETRY CTA
// ─────────────────────────────────────────────────────────────
function ErrorState({
  icon = "txfail",
...
//   lines: how many skeleton "text lines" inside (default 2)
//   leftAccent: bool — prepend a 3px olive bar (matches W/L history style)
// ─────────────────────────────────────────────────────────────
function SkeletonRow({ height = 32, lines = 2, leftAccent = false, style = {} }) {
  return (
    <div style={{
      ...
      {/* Optional avatar/icon block */}
      <div style={{
        width: height - 10, height: height - 10,
        border: "1px dashed var(--muted)",
        flex: "0 0 auto",
      }} />
      {/* Stacked lines */}
...
//   variant: "stat" | "tile" | "hero" — preset internal layouts
// ─────────────────────────────────────────────────────────────
function SkeletonCard({ width, height, variant = "tile", style = {} }) {
  return (
    <div style={{
      width, height,
      background: "var(--bg-deep)",
...
// One-time pulse keyframes injection (idempotent)
...
Object.assign(window, {
  EmptyState, ErrorState, SkeletonRow, SkeletonCard, Icon, CTA, CornerBracket, ICONS,
});
