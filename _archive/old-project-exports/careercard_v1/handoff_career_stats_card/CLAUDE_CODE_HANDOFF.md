...
| `CareerStatsCard_Preview.html` | Standalone preview with **3 preset states** (strong/mid/fresh). |
...
5. **Stat tiles are 4-across and tighter** than the trophy card (smaller numbers, smaller padding). This card is a reference, not a celebration; the data should feel cataloged, not shouted.
6. **Rank pill is bone-on-orange** when ranked, **outlined olive** when unranked. Same spot, two visual weights — the eye finds it identically.
7. **UNRANKED state is a `[CLASSIFIED]` plate** with "TIER PENDING / EARN A WIN" subtext, replacing the badge. Gives fresh players a reason to come back without making them feel like a mistake.
...
- **Stat tile big-number font scales by string length** (smallBig=true for K/D and MVP WEAPON which are wider strings). If you add new stats, follow the same pattern — never let a number overflow.

---

## § E — Acceptance checklist

- [ ] Renders identically to the three presets in `CareerStatsCard_Preview.html` (strong / mid / fresh).
- [ ] PNG is exactly **1080×608**, file size <300KB (badges are heavy — see § F).
- [ ] Long callsigns (`OVERLORD-99`) clip without breaking layout.
