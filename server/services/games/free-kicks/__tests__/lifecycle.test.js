...
test('createMatch is deterministic — same inputs yield same shape', () => {
    const a = createMatch({
        matchId: 'freekicks:m1', roomId: 'm1',
...
test('createMatch sets correct matchId prefix for free-kicks', () => {
    const m = freshMatch();
    assert.ok(m.matchId.startsWith('freekicks:'),
        `expected matchId to start with "freekicks:", got "${m.matchId}"`);
});
