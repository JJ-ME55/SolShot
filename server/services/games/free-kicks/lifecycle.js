...
/**
 * Free-Kick Madness — match lifecycle state machine
 *
 ...
 *
 * NOTE: This state machine is intentionally identical in shape and
 * semantics to `server/services/games/basketball/lifecycle.js`. The
 * match-format pattern (deposits → time window → settle/OT) is the
 * SolShot arcade default for any best-of-window wagered title. A
 * future refactor may promote it to a shared module; for now
 * duplication is the lower-risk move per the playbook.
 */
 ...
 *
 * @param {object} params
 * @param {string} params.matchId - "freekicks:<roomId>"
 ...
 *
 * For free-kicks, an "attempt" is a full run from the player's first
 * shot through to lives=0 (or whatever ended their run). finalScore
 * is the total points accumulated during the run.
 */
 ...
 *
 * For free-kicks, an OT attempt is a sudden-death single-shot:
 * one curl, whoever scores OR scores higher wins. Caller decides
 * the OT rules (single-shot, 3-shot tiebreak, etc.) and passes the
 * resolved finalScore here.
 */
